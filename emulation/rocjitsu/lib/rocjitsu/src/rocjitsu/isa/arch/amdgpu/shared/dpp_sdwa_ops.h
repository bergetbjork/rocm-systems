...
enum DppCtrl : uint32_t {
  ...
  ROW_BCAST15 = 0x142,    // broadcast lane 15/47 to the following row
...
inline int dpp_permute(uint32_t dpp_ctrl, int lane, int wf_size, bool &out_of_bounds) {
  ...
  if (dpp_ctrl == ROW_BCAST15) {
    // Broadcast lane 15 of each half-wave to lanes 16-31, and lane 47 to
    // lanes 48-63. Lanes in the lower row of each half-wave have invalid
    // shared data.
    if ((lane & 16) == 0) {
      out_of_bounds = true;
      return lane;
