...
// Like registerInputWindows() but returns the registration result instead of
// asserting success. Used by the negative tests: in NCCL 2.30 an unsupported
// configuration (cuMem / symmetric windows disabled) is rejected at
// ncclCommWindowRegister, whereas older releases accepted registration and
// rejected later at ncclDevCommCreate. Returns the first non-success per-rank
// result, otherwise the ncclGroupEnd() result.
