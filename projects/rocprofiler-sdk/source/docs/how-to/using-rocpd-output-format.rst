...
PC sampling data (schema 3.0.2)
---------------------------------

PC sampling support was introduced in schema version **3.0.2**. When
...
.. list-table::
  ...
  * - ``correlation_id``
    - INTEGER
    - External correlation ID linking back to the API launch call.
  * - ``exec_mask``
    - INTEGER
    - Bitmask of active SIMD lanes at sample time.
  * - ``code_object_id``
    - INTEGER
...
columns from ``rocpd_gpu_pc_sample``. The view is created on demand by
``setup_blob_views``, which reads ``rocpd_info_blob_schema`` at startup,
locates the ``pc_sample_extdata_v1`` registration with
``source_table = 'rocpd_gpu_pc_sample'``, and generates the decoded view.
No blob bytes are read unless the view is actually queried; queries against
``rocpd_gpu_pc_sample`` directly incur zero unpacking overhead.
