...
- ``rocpd_gpu_pc_sample``: One row per sample. Key columns:
  ...
  - ``correlation_id``: External correlation ID linking back to the API call
  - ``exec_mask``: Active SIMD lanes at sample time
...
``rocpd_event.id`` of the corresponding kernel dispatch. This allows you to join
samples to dispatch metadata:
...
The ROCPD schema version containing PC sampling support is **3.0.2**. For the
